...
class TestMigrationScript(unittest.TestCase):
    ...
    def test_process_gc_data(self):
        ...
        all_activities1 = get_all_activity_for_member(member1[0])
        gc_activities1 = [a for a in all_activities1 if a[0] == 'Group Class']
        self.assertEqual(len(gc_activities1), 1)
        gc_activity = gc_activities1[0]
        self.assertEqual(gc_activity[2], "2024-01-01") # payment_date
        self.assertEqual(gc_activity[3], "2024-01-01") # start_date
        self.assertEqual(gc_activity[5], 5000.0)       # amount_paid
        plan1 = get_plan_by_name_and_duration("Standard", 3)
        self.assertIsNotNone(plan1)
        # verify plan name matches
        self.assertEqual(gc_activity[1], plan1[1])     # name_or_description should be plan_name

        member2 = get_member_by_phone("0987654321")
        self.assertIsNotNone(member2, "Test User 2 (GC) should be found")
        all_activities2 = get_all_activity_for_member(member2[0])
        gc_activities2 = [a for a in all_activities2 if a[0] == 'Group Class']
        ...
        self.assertEqual(gc_activities2[0][1], plan2[1])  # name_or_description should be plan_name
    ...
    def test_process_pt_data(self):
        ...
        pt_activities_pt1 = [a for a in all_activities_pt1 if a[0] == 'Personal Training']
        self.assertEqual(len(pt_activities_pt1), 1)
        pt_activity = pt_activities_pt1[0]
        self.assertEqual(pt_activity[3], "2024-01-01") # start_date
        self.assertEqual(pt_activity[6], "10 sessions") # payment_method_or_sessions for PT
        self.assertEqual(pt_activity[5], 3000.0)      # amount_paid
        ...
        gc_activities_gc_pt = [a for a in all_activities_gc_pt if a[0] == 'Group Class']
        ...
        pt_activities_gc_pt = [a for a in all_activities_gc_pt if a[0] == 'Personal Training']
        self.assertEqual(len(pt_activities_gc_pt), 1, "Test User 2 should have a PT booking")
        pt_booking = pt_activities_gc_pt[0]
        self.assertEqual(pt_booking[3], "2024-03-10") # start_date for PT
        self.assertEqual(pt_booking[6], "5 sessions")  # payment_method_or_sessions for PT
        self.assertEqual(pt_booking[5], 1500.0)       # amount_paid for PT
